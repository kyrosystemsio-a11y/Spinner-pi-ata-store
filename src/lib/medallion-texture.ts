...
// code. "standard" is an all-charcoal medallion; "gold" swaps only the
// outer beaded ring + hairline to a gold gradient (the cross itself stays
// charcoal in both variants, per spec).
export type MedallionVariant = "standard" | "gold";
...
function drawCenterAccent(ctx: CanvasRenderingContext2D, cx: number, cy: number, scale: number) {
...
// Cross is always charcoal regardless of variant — only the outer ring +
// hairline change color (see createMedallionTexture).
function drawCross(ctx: CanvasRenderingContext2D, cx: number, cy: number, scale: number) {
  ...
  const waistWidth = scale * 0.085;
  const flareWidth = scale * 0.2;
  ...
  drawCenterAccent(ctx, cx, cy, scale * 0.16);
}

export function createMedallionTexture(variant: MedallionVariant): THREE.CanvasTexture {
  ...
  const outerR = TEXTURE_SIZE * 0.46;

  let ringStyle: string | CanvasGradient = CHARCOAL;
  if (variant === "gold") {
  ...
  drawBeadedRing(ctx, cx, cy, outerR, ringStyle);
  drawHairline(ctx, cx, cy, outerR * 0.88, ringStyle);
  drawCross(ctx, cx, cy, outerR * 0.57);
